...
function buildWhatsAppLink(whatsappNumber: string, d: FormData) {
  const lines = [
    `*Contacto · Kuro Sushi*`,
    ``,
    `Nombre: ${d.name}`,
...
export function UbicacionPageClient({
  settings,
  hours,
}: {
  settings: Settings;
  ...
  const onSubmit = (data: FormData) => {
    const url = buildWhatsAppLink(settings.whatsappNumber, data);
      ...
      <section className="pt-36 md:pt-44 pb-12 bg-kuro-black">
        <div className="max-w-[1280px] mx-auto px-gutter">
          ...
          <p className="mt-6 text-kuro-stone text-base md:text-lg max-w-xl leading-relaxed">
            {settings.address.street} — {settings.address.city}. Fácil acceso y
